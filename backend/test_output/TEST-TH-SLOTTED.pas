{ Altium DelphiScript - Create Footprint: TEST-TH-SLOTTED }
{ Generated by FootprintAI }
{
  Footprint: TEST-TH-SLOTTED
  Description: Through-hole test with slotted holes
  Pads: 3
  Vias: 0
}

Procedure CreateFootprint_TEST_TH_SLOTTED;
Var
    Board      : IPCB_Board;
...
Begin
    ...
    NewComp.Name := 'TEST-TH-SLOTTED';
    NewComp.Description := 'Through-hole test with slotted holes';
    ...
    If Pad <> Nil Then
    Begin
        Pad.X := MMsToCoord(-5.0000);
        Pad.Y := MMsToCoord(0.0000);
        Pad.Name := 'SH1';
        Pad.Layer := eMultiLayer;
        Pad.Mode := ePadMode_Simple;
        Pad.TopShape := eRounded;
        Pad.TopXSize := MMsToCoord(3.0500);
        Pad.TopYSize := MMsToCoord(1.2500);
        Pad.Rotation := 90.000;
        Pad.HoleSize := MMsToCoord(0.6500);
        Pad.HoleType := eSlotHole;
        Pad.HoleSize := MMsToCoord(0.6500);
        Pad.HoleWidth := MMsToCoord(2.4500);
        // Slot: 0.65mm width x 2.45mm length
        NewComp.AddPCBObject(Pad);
    End;
    ...
    If Track <> Nil Then
    Begin
        Track.X1 := MMsToCoord(-6.0000);
        Track.Y1 := MMsToCoord(-2.5000);
        Track.X2 := MMsToCoord(6.0000);
        Track.Y2 := MMsToCoord(-2.5000);
    ...
    If Arc <> Nil Then
    Begin
        Arc.XCenter := MMsToCoord(0.5000);
        Arc.YCenter := MMsToCoord(2.5000);
    ...
    ShowMessage('Footprint TEST-TH-SLOTTED created successfully!');
End;

{ Main entry point }
Procedure Run;
Begin
    CreateFootprint_TEST_TH_SLOTTED;
End;
